...
class StorageService {
  ...
  Future<void> saveLastPosition(String songId, Duration position) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('${_lastPositionKey}_$songId', position.inMilliseconds.toString());
  }

  Future<Duration?> getLastPosition(String songId) async {
    final prefs = await SharedPreferences.getInstance();
    final positionString = prefs.getString('${_lastPositionKey}_$songId');
    if (positionString != null) {
      try {
        final milliseconds = int.parse(positionString);
